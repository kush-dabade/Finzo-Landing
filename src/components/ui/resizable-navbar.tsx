...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      animate={{
        boxShadow: visible ? "0 1px 0 rgba(255,255,255,0.06)" : "none",
      }}
      transition={{
        duration: 0.25,
        ease: "easeOut",
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  return (
    <div
      className={cn(
        "absolute inset-0 hidden flex-1 items-center justify-center space-x-6 text-sm font-medium lg:flex",
        className,
      )}
    >
      {items.map((item, idx) => (
...
export const NavbarLogo = () => {
  return (
    ...
    >
      <span className="font-semibold text-white tracking-tight">Finzo</span>
    </a>
  );
};
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md text-sm font-medium relative cursor-pointer transition inline-flex items-center justify-center";

  const variantStyles = {
    primary: "bg-white text-black hover:bg-white/90",
    secondary: "bg-transparent text-white/80 hover:text-white hover:bg-white/5",
